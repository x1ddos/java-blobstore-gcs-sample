...
public class ServeUrl {
    ...
    /**
     * Google Cloud Storage (GCS) base URL.
     * See Request URIs for more details:
     ...
     * </ul>
     * 
     * Reading blobs uploaded to a GCS bucked-based URLs is not currenly supported
     * in dev server, so in "dev" mode all serving URL will have a value
     * of <code>"/dev-does-not-support-gcs-serving-yet"</code>.
     * 
    ...
    /**
     * Creates a URL which is served by {@link ServeBlobServlet}.
     * Note that App Engine app response limit is 32Mb so files larger than
     * 32Mb will throw errors during serving.
     * 
     * @param file Typically obtained from BlobstoreService.getFileInfos().
     */
    public String getServletServingPath(FileInfo file) {
        try {
            return "/serve/" + URLEncoder.encode(file.getGsObjectName(), "UTF-8");
     ...
     * 
     * @param file Typically obtained from BlobstoreService.getFileInfos(). 
     * @return URL that points to Google GCS, e.g bucket.storage.googleapis.com/object
     */
